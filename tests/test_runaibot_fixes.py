...
def test_ai_cannot_answer_text_question_prompts_manual_continue(bot, monkeypatch):
    ...
    monkeypatch.setattr(bot, "ai_provider", "openai")
    monkeypatch.setattr(bot, "answer_question", lambda *a, **k: "", raising=False)
    monkeypatch.setattr(bot, "human_type", lambda target, text: typed.append((target, text)), raising=False)
    monkeypatch.setattr(bot, "print_lg", lambda *a, **k: None)
    monkeypatch.setattr(bot, "interactive_session", True)
    monkeypatch.setattr(bot.pyautogui, "confirm", lambda text, title="", choices=None: alerted.append((text, title, choices or ["Continue"])))
    ...
    assert alerted, "The unanswered AI question should raise a blocking manual choice dialog."
    assert typed == [], "The unanswered text field must not be auto-typed with an empty answer."


def test_cleanup_banner_and_signature_text_is_removed_from_active_bot_source():
    source = open(os.path.join(os.path.dirname(__file__), os.pardir, "runAiBot.py"), encoding="utf-8").read()
    # Only the runtime banner/signoff block should be absent. Keep metadata intact.
    for banned in [
        "Top Sponsors:",
        "Best regards,",
        "You can also get your quote and name shown here",
        "Closing the browser...",
    ]:

...
def test_control_panel_failed_jobs_filters_placeholder_unknown_rows(client, tmp_path, monkeypatch):
    import app
    monkeypatch.setattr(app, "PATH", str(tmp_path) + os.sep)
    csv_path = tmp_path / "all_failed_applications_history.csv"
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        ...
        writer.writerow({
            'Job ID': '', 'Job Link': '', 'Resume Tried': '', 'Date listed': '', 'Date Tried': '',
            'Assumed Reason': 'Not applied / stopped', 'Stack Trace': '', 'External Job link': '', 'Screenshot Name': '',
            'Company': 'Unknown', 'Title': 'Unknown', 'Status': 'Not applied / stopped', 'HR Name': 'Unknown', 'HR Link': 'Unknown',
        })
...
def test_user_config_search_experience_level_uses_valid_labels():
    import config._overrides as overrides
    data = overrides.load_user_config()
    search = data.get("search", {})
    level = search.get("experience_level", [])
    assert all(item in ["Internship", "Entry level", "Associate", "Mid-Senior level", "Director", "Executive"] for item in level)

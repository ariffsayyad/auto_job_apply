...
def _clear_manual_question():
    try:
        os.remove(MANUAL_QUESTION_PATH)
    except OSError:
        pass
...
def get_failed_jobs():
    ...
    Ignore rows that are only the writer's placeholder defaults. Those rows are not
    actual company/job records, so they must not be rendered in the history view.
    """
    csv_path = os.path.join(PATH, _FAILED_HISTORY_CSV)
    if not os.path.exists(csv_path):
        return jsonify([])
    try:
        with open(csv_path, 'r', encoding='utf-8') as f:
...
def api_manual_question():
    ...
    if action not in {"continue", "stop", "next"}:
        return jsonify({"error": "action must be continue, stop, or next"}), 400
    if not os.path.exists(MANUAL_QUESTION_PATH):
        return jsonify({"error": "There is no unanswered question waiting."}), 409
    try:
        with open(MANUAL_QUESTION_PATH, "r", encoding="utf-8") as question_file:
            state = json.load(question_file)
        state["action"] = action
        ...
        return jsonify({"error": f"Could not save the decision: {err}"}), 500

...
def pause_for_manual_continue(label_org: str, question_type: str = "question") -> str:
    ...
    message = (
        f'Could not answer the {question_type} question automatically: "{label_org}".\n'
        'Please answer it manually in the LinkedIn modal, then choose what to do next.'
    )
    choices = ["Continue", "Stop this application", "Skip to next application"]
    try:
...
manual_question_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".manual_question.json")
...
def failed_job(job_id: str, job_link: str, resume: str, date_listed, error: str, exception: Exception, application_link: str, screenshot_name: str,
               company: str = 'Unknown', title: str = 'Unknown', status: str = 'Not applied / stopped') -> None:
    '''
    Function to update failed jobs list in excel. It writes the same structured
    fields the control-panel failed-job table expects: Company, Title, Status,
    HR fields, and the job/application links if the bot already knows them.
    '''
    try:
        with open(failed_file_name, 'a', newline='', encoding='utf-8') as file:
            fieldnames = ['Job ID', 'Job Link', 'Resume Tried', 'Date listed', 'Date Tried',
                          ...
                          'Company', 'Title', 'Status', 'HR Name', 'HR Link']
            ...
            record = {
                ...
                'HR Name': 'Unknown', 'HR Link': 'Unknown',
            }
                    ...
                    if is_easy_apply:
                        try: 
                            ...
                            try:
                                ...
                                while next_button:
                                    ...
                                    if next_counter >= 15: 
                                        if pause_at_failed_question:
                                            ...
                                            decision = pause_for_manual_continue("one or more questions in this application", "question")
                                            if decision == "Stop this application":
                                                print_lg('User chose to stop this application while a question remained unanswered.')
                                                discard_reason = "User stopped this application after an unanswered question."
                                                raise Exception(discard_reason)
                                            if decision == "Skip to next application":
                                                print_lg('User chose to skip this application and continue with the next one.')
                                                skip_count += 1
                                                discard_job()
                                                break
                                            next_counter = 1
                                    ...
                                    if manual_question_decision in {"Stop this application", "Skip to next application"}:
                                        decision = manual_question_decision
                                        manual_question_decision = "Continue"
                                        if decision == "Stop this application":
                                            print_lg('User chose to stop this application after an unanswered question.')
                                            errored = "manual_stop"
                                            break
                                        print_lg('User chose to skip this application and continue with the next one.')
                                        errored = "manual_skip"
                                ...
                                if errored in {"stuck", "manual_stop", "manual_skip"}:
...
def main() -> None:
    ...
    try:
        ...
        # Validate user configuration FIRST so a bad config never opens a browser
        # and then aborts a couple of seconds later with a generic close.
        validate_config()

        # Create the browser session only after the config has passed validation.
        try:
            ...
            if isinstance(e, TimeoutError):
                msg = "Couldn't download Chrome-driver. Set auto_manage_driver = False in config!"
            print_lg(msg)
        ...
        pyautogui.alert("Run summary complete.\n\n" + summary, "Exiting..")
        print_lg(summary)
        if tabs_count >= 10:
